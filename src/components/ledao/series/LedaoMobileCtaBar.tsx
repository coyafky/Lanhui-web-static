...
/**
 * 乐道页移动端底部固定双 CTA 栏（lg:hidden）：按车型查看 / 咨询适配方案。
 */
export function LedaoMobileCtaBar() {
  const scrollToModels = () => {
    document
      .getElementById("ledao-models")
    ...
    <div className="fixed bottom-0 inset-x-0 z-40 lg:hidden pb-[env(safe-area-inset-bottom,0px)]">
      <div className="flex gap-3 bg-zinc-950/95 backdrop-blur border-t border-white/[0.08] px-4 py-3">
        ...
        <button
          ...
          className="flex-1 inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-gradient-to-r from-blue-500 to-blue-600 px-4 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 active:scale-[0.97] transition-transform"
        >
