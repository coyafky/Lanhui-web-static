import { ledaoSeriesServiceSteps } from "@/lib/ledao-series-services";

/**
 * 服务流程与施工边界：6 步（确认车型 → 检查原车 → 方案确认 → 保护施工 → 功能复检 → 交付养护）。
 */
export function LedaoServiceFlow() {
  return (
    <section
      aria-labelledby="ledao-flow-title"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <ol className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {ledaoSeriesServiceSteps.map((step) => (
            <li
              key={step.step}
              className="rounded-2xl bg-zinc-900/60 p-5 shadow-[0_0_0_1px_oklch(1_0_0/0.06)]"
            >
              <span className="inline-flex size-8 items-center justify-center rounded-lg bg-blue-400/10 text-sm font-bold text-blue-400">
                {step.step}
              </span>
        ...
        <div className="mt-6 rounded-2xl bg-blue-950/20 p-5 shadow-[0_0_0_1px_oklch(0.546_0.245_262.9/0.15)]">
          <p className="text-sm text-zinc-300 leading-relaxed text-pretty">
            施工边界：涉及电路、座椅滑轨、门体、底盘、轮毂载荷或原车功能的项目，
            会在施工前逐项书面告知影响范围，你确认后才动工。
            可为乐道全系提供基础服务咨询，电动踏板、地板、轮毂及其他专车项目需根据车型、年款、配置、原车状态和实际安装条件确认，并非所有车型通用。
          </p>
        </div>
      </div>
    </section>
