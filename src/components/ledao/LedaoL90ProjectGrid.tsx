...
import {
  ...
  type LedaoL90Scenario,
  type LedaoL90UpgradeProject,
} from "@/lib/ledao-l90-products";

const EXPECTED_PROJECT_COUNT = 21;

const CATEGORY_ORDER: readonly LedaoL90Category[] = [
  "paint_protection",
  "film_style",
  "exterior_parts",
  "exterior_style",
...
function assertProjectCount(projects: readonly LedaoL90UpgradeProject[]): void {
  if (projects.length !== EXPECTED_PROJECT_COUNT) {
    throw new Error(
      `LedaoL90ProjectGrid expects ${EXPECTED_PROJECT_COUNT} projects, got ${projects.length}`,
    );
  }
}

type LedaoL90ProjectGridProps = {
  projects: readonly LedaoL90UpgradeProject[];
  scenarios: readonly LedaoL90Scenario[];
};

type ProjectCardProps = {
  project: LedaoL90UpgradeProject;
  open: boolean;
  onToggle: () => void;
};
  ...
  return (
    <article className="group bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col">
      <button
        ...
        onClick={handleClick}
        aria-expanded={open}
        aria-controls={`ledao-l90-project-detail-${project.id}`}
        ...
        <div className="p-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="inline-flex items-center px-2 py-0.5 rounded-md border border-orange-900/60 text-orange-400 bg-orange-950/30 text-xs">
              {LEDAO_L90_CATEGORY_LABELS[project.category]}
            </span>
      ...
      <div
        id={`ledao-l90-project-detail-${project.id}`}
...
export function LedaoL90ProjectGrid({
  projects,
  scenarios,
}: LedaoL90ProjectGridProps) {
  assertProjectCount(projects);

  const sectionRef = useRef<HTMLElement>(null);
  const [activeCategory, setActiveCategory] = useState<LedaoL90Category | "all">(
    "all",
  );
  ...
  const handleHashChange = useCallback(() => {
    ...
    const projectMatch = hash.match(/^ledao-l90-project-(.+)$/);
    if (projectMatch) {
  ...
  const scenarioFilteredProjects = useMemo<readonly LedaoL90UpgradeProject[]>(
    () => {
  ...
  const filteredProjects = useMemo<readonly LedaoL90UpgradeProject[]>(() => {
        ...
        >
          ...
          {CATEGORY_ORDER.map((cat) => {
            ...
            return (
              ...
              >
                {LEDAO_L90_CATEGORY_LABELS[cat]}（{count}）
              </button>
            );
          })}
        </div>
