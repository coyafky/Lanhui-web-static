...
import type { DenzaD9FaqItem } from "@/lib/denza-d9-products";

type DenzaD9FaqProps = {
  items: readonly DenzaD9FaqItem[];
};

const FAQ_LENGTH = 6;

function assertFaqLength(items: readonly DenzaD9FaqItem[]): void {
  if (items.length !== FAQ_LENGTH) {
    throw new Error(
      `DenzaD9Faq expects ${FAQ_LENGTH} items, got ${items.length}`,
    );
  }
}

/**
 * 6 条 FAQ（Client Component）— 一次只展开一项
 */
export function DenzaD9Faq({ items }: DenzaD9FaqProps) {
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-3">
          {items.map((item, idx) => {
            ...
            const panelId = `denza-d9-faq-panel-${idx}`;
            return (
              <div
                key={item.question}
