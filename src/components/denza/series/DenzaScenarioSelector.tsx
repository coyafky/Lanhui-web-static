...
import {
  ...
  denzaBaseServices,
  type DenzaScenarioEntryId,
} from "@/lib/denza-series-services";
...
/**
 * 4 个高频场景选择器（MPV 版：家庭出行 / 商务接待）：点击高亮 → 显示推荐服务组合。
 * H2 使用问题型标题（GEO）。
 */
export function DenzaScenarioSelector() {
  const [activeId, setActiveId] = useState<DenzaScenarioEntryId>(
    denzaScenarioEntries[0].id,
  );
  const active =
    denzaScenarioEntries.find((s) => s.id === activeId) ??
    denzaScenarioEntries[0];
  const services = active.serviceIds
    .map((id) => denzaBaseServices.find((s) => s.id === id))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));

  return (
    <section
      id="denza-scenarios"
      aria-labelledby="denza-scenarios-title"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 lg:mb-10">
          ...
          <p className="mt-3 text-zinc-400 max-w-2xl text-base leading-relaxed text-pretty">
            先解决暴晒、上下车和后排清洁这些高频问题，再按需求加项。选一个最接近你日常的场景，我们给出对应的服务组合参考。
          </p>
        </div>
