...
/**
 * 6 类基础服务（全系腾势均可咨询）：解决什么 / 适合谁 / 进入对应产品页。
 * 车膜类卡片包含车衣、隔热膜、改色膜 3 个子入口。
 * H2 使用问题型标题 + 直接答案段（GEO）。
 */
export function DenzaBaseServiceGrid() {
  return (
    <section
      aria-labelledby="denza-services-title"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 lg:mb-10">
          ...
          <p className="mt-3 text-zinc-400 max-w-2xl text-base leading-relaxed text-pretty">
            车膜、轮毂、电动踏板、地板总成、专车脚垫和洗美养护这 6 类服务全系腾势均可咨询。
            每类都链接到独立的专业页面，先了解清楚再决定；涉及座椅、滑轨等结构差异的项目按车型确认。
          </p>
        </div>

        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {denzaBaseServices.map((svc) => {
            ...
            return (
              ...
              >
                <div className="size-10 rounded-xl bg-orange-400/10 flex items-center justify-center mb-4">
                  {Icon && <Icon className="size-5 text-orange-400" aria-hidden />}
                </div>
                ...
                {svc.subLinks ? (
                  ...
                  <Link
                    href={svc.href}
                    className="mt-4 inline-flex min-h-11 items-center gap-2 self-start rounded-full bg-white/[0.04] px-5 text-sm font-medium text-orange-300 shadow-[0_0_0_1px_oklch(1_0_0/0.08)] transition-colors hover:bg-white/[0.08] active:scale-[0.96]"
                  >
                    了解{svc.title}
