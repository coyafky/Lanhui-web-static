...
import type { XpengGxFaqItem } from "@/lib/xpeng-gx-products";

type XpengGxFaqProps = {
  items: readonly XpengGxFaqItem[];
};

const FAQ_LENGTH = 8;

function assertFaqLength(items: readonly XpengGxFaqItem[]): void {
  if (items.length !== FAQ_LENGTH) {
    throw new Error(
      `XpengGxFaq expects ${FAQ_LENGTH} items, got ${items.length}`,
    );
  }
}

/**
 * 8 条 FAQ（Client Component）— 一次只展开一项
 * 结构一比一对齐极氪 9X FAQ。
 */
export function XpengGxFaq({ items }: XpengGxFaqProps) {
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-expanded={open}
                  aria-controls={`xpeng-gx-faq-panel-${idx}`}
                >
