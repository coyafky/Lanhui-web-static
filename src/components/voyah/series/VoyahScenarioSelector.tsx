...
import {
  voyahScenarioEntries,
  voyahBaseServices,
  type VoyahScenarioEntryId,
 ...
 */
export function VoyahScenarioSelector() {
  const [activeId, setActiveId] = useState<VoyahScenarioEntryId>(
    voyahScenarioEntries[0].id,
  );
  const active =
    voyahScenarioEntries.find((s) => s.id === activeId) ??
    voyahScenarioEntries[0];
  const services = active.serviceIds
    .map((id) => voyahBaseServices.find((s) => s.id === id))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));

  return (
    <section
      id="voyah-scenarios"
      aria-labelledby="voyah-scenarios-title"
            ...
            return (
              <button
                ...
                className={`flex min-h-11 flex-col items-start gap-1 rounded-2xl p-4 text-left transition-colors active:scale-[0.98] ${
                  isActive
                    ? "bg-violet-400/10 shadow-[0_0_0_1px_oklch(0.606_0.25_292.7/0.4)]"
              ...
              >
                <span className="flex items-center gap-2">
                  {Icon && (
                    <Icon
                      className={`size-4 ${isActive ? "text-violet-300" : "text-zinc-400"}`}
                      aria-hidden
                    />
                  )}
                  <span
                    className={`text-base font-semibold ${isActive ? "text-violet-300" : "text-white"}`}
                  >
                    {s.title}
                  </span>
                </span>
        ...
        <div className="mt-6 rounded-3xl bg-zinc-900/60 p-6 shadow-[0_0_0_1px_oklch(1_0_0/0.06)]">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {services.map((svc) => (
              ...
              >
                {svc.title}
                <ArrowRight className="size-4 text-violet-400" aria-hidden />
              </Link>
            ))}
          </div>
