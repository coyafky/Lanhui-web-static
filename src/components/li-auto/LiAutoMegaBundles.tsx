...
import type {
  LiAutoMegaBundle,
  LiAutoMegaUpgradeProject,
} from "@/lib/li-auto-mega-products";

export type LiAutoMegaBundlesProps = {
  bundles: readonly LiAutoMegaBundle[];
  allProjects: readonly LiAutoMegaUpgradeProject[];
  modelKey: string;
};
 ...
 * 理想 MEGA 5 个推荐组合（RSC）
 * plan §C.3：
 *   - 5 张组合卡片（grid-cols-1/md:2/lg:4，第 5 张折行）
 *   - amber 主题色
 *   - 每张卡片底部「咨询此组合」CTA（Link → #consult + bundle key）
 *   - 项目标签纯展示，不可点击（RSC 无 hash 联动）
 */
export function LiAutoMegaBundles({
  bundles,
  allProjects,
  modelKey,
}: LiAutoMegaBundlesProps) {
