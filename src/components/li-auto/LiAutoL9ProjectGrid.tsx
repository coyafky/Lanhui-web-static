...
const EXPECTED_PROJECT_COUNT = LI_AUTO_L9_PROJECT_COUNT;
const SCENARIO_KEYS: readonly LiAutoL9ScenarioKey[] = [
  "new_car_protection",
  "family_cabin",
  "appearance",
  "driving_protection",
...
const SCENARIO_TAB_LABELS: Record<LiAutoL9ScenarioKey, string> = {
  new_car_protection: "新车保护",
  family_cabin: "家庭座舱",
  appearance: "外观个性",
...
function assertProjectCount(
  projects: readonly LiAutoL9UpgradeProject[],
): void {
  if (projects.length !== EXPECTED_PROJECT_COUNT) {
    throw new Error(
      `LiAutoL9ProjectGrid expects ${EXPECTED_PROJECT_COUNT} projects, got ${projects.length}`,
    );
  }
}

export type LiAutoL9ProjectGridProps = {
  projects: readonly LiAutoL9UpgradeProject[];
  scenarios: readonly LiAutoL9Scenario[];
  modelKey: string;
};

type ProjectCardProps = {
  project: LiAutoL9UpgradeProject;
  open: boolean;
  onToggle: () => void;
  scenarioNameByKey: Readonly<Record<LiAutoL9ScenarioKey, string>>;
};
...
function ProjectCard({
  ...
  const scenarioLabel = project.suitableFor
    .map((s) =>
      (SCENARIO_KEYS as readonly string[]).includes(s)
        ? scenarioNameByKey[s as LiAutoL9ScenarioKey]
        : null,
    )
  ...
  return (
    <article className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col">
      ...
      <div
        id={`li-auto-l9-project-detail-${project.key}`}
...
/**
 * 理想 L9 14 项升级项目网格（Client）
 * plan §C.2：
 *   - 5 场景 tab 切换（默认"全部"）
 *   - 每张卡可点击展开 detail panel
 */
export function LiAutoL9ProjectGrid({
  projects,
  scenarios,
  modelKey,
}: LiAutoL9ProjectGridProps) {
  assertProjectCount(projects);

  const scenarioNameByKey = useMemo<
    Readonly<Record<LiAutoL9ScenarioKey, string>>
  >(() => {
    const map: Record<LiAutoL9ScenarioKey, string> = {
      new_car_protection: "",
      family_cabin: "",
      appearance: "",
      driving_protection: "",
  ...
  const [activeTab, setActiveTab] = useState<LiAutoL9ScenarioKey | "all">(
    "all",
  );
  const [openKey, setOpenKey] = useState<string | null>(null);

  // hash 高亮（来自 LiAutoL9Bundles 的 hashchange）
  useEffect(() => {
    const sync = () => {
  ...
  const filteredProjects = useMemo<readonly LiAutoL9UpgradeProject[]>(() => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          {scenarios.map((s) => {
            ...
            const tabLabel =
              SCENARIO_TAB_LABELS[s.key as LiAutoL9ScenarioKey] ?? s.name;
            return (
              <button
                key={s.key}
                type="button"
