...
import {
  liAutoScenarioEntries,
  liAutoBaseServices,
  type LiAutoScenarioEntryId,
...
/**
 * 5 个高频家庭场景选择器：点击高亮 → 显示推荐服务组合（链接到对应产品页）。
 */
export function LiAutoScenarioSelector() {
  const [activeId, setActiveId] = useState<LiAutoScenarioEntryId>(
    liAutoScenarioEntries[0].id,
  );
  const active =
    liAutoScenarioEntries.find((s) => s.id === activeId) ??
    liAutoScenarioEntries[0];
  const services = active.serviceIds
    .map((id) => liAutoBaseServices.find((s) => s.id === id))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));

  return (
    <section
      id="li-auto-scenarios"
      aria-labelledby="li-auto-scenarios-title"
            ...
            return (
              <button
                ...
                className={`flex min-h-11 flex-col items-start gap-1 rounded-2xl p-4 text-left transition-colors active:scale-[0.98] ${
                  isActive
                    ? "bg-amber-400/10 shadow-[0_0_0_1px_oklch(0.769_0.188_70.08/0.4)]"
              ...
              >
                <span className="flex items-center gap-2">
                  {Icon && (
                    <Icon
                      className={`size-4 ${isActive ? "text-amber-300" : "text-zinc-400"}`}
                      aria-hidden
                    />
                  )}
                  <span
                    className={`text-base font-semibold ${isActive ? "text-amber-300" : "text-white"}`}
                  >
                    {s.title}
                  </span>
                </span>
        ...
        <div className="mt-6 rounded-3xl bg-zinc-900/60 p-6 shadow-[0_0_0_1px_oklch(1_0_0/0.06)]">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {services.map((svc) => (
              ...
              >
                {svc.title}
                <ArrowRight className="size-4 text-amber-400" aria-hidden />
              </Link>
            ))}
          </div>
