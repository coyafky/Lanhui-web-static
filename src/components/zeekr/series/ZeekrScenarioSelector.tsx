 ...
 */
export function ZeekrScenarioSelector() {
  const [activeId, setActiveId] = useState<ZeekrScenarioEntryId>(
    zeekrScenarioEntries[0].id,
  );
  const active =
    zeekrScenarioEntries.find((s) => s.id === activeId) ??
    zeekrScenarioEntries[0];
  const services = active.serviceIds
    .map((id) => zeekrBaseServices.find((s) => s.id === id))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));

  return (
    <section
      id="zeekr-scenarios"
      aria-labelledby="zeekr-scenarios-title"
