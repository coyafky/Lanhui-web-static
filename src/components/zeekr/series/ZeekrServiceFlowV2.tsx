import { zeekrSeriesServiceSteps } from "@/lib/zeekr-series-services";

/**
 * 服务流程与施工边界：6 步（车型核对 → 方案边界 → 到店评估 → 施工保护 → 功能复检 → 交付售后）。
 */
export function ZeekrServiceFlowV2() {
  return (
    <section
      aria-labelledby="zeekr-flow-title"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <ol className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {zeekrSeriesServiceSteps.map((step) => (
            <li
              key={step.step}
        ...
        <div className="mt-6 rounded-2xl bg-orange-950/20 p-5 shadow-[0_0_0_1px_oklch(0.705_0.213_47.6/0.15)]">
          <p className="text-sm text-zinc-300 leading-relaxed text-pretty">
            ...
            你确认后才动工。新能源车型的传感器、线束和座椅结构差异较大，方案以你的具体车辆为准。
          </p>
        </div>
      </div>
    </section>
