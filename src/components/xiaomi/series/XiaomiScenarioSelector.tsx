...
import {
  xiaomiScenarioEntries,
  xiaomiBaseServices,
  type XiaomiScenarioEntryId,
 ...
 */
export function XiaomiScenarioSelector() {
  const [activeId, setActiveId] = useState<XiaomiScenarioEntryId>(
    xiaomiScenarioEntries[0].id,
  );
  const active =
    xiaomiScenarioEntries.find((s) => s.id === activeId) ??
    xiaomiScenarioEntries[0];
  const services = active.serviceIds
    .map((id) => xiaomiBaseServices.find((s) => s.id === id))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));

  return (
    <section
      id="xiaomi-scenarios"
      aria-labelledby="xiaomi-scenarios-title"
            ...
            return (
              <button
                ...
                className={`flex min-h-11 flex-col items-start gap-1 rounded-2xl p-4 text-left transition-colors active:scale-[0.98] ${
                  isActive
                    ? "bg-orange-400/10 shadow-[0_0_0_1px_oklch(0.705_0.213_47.604/0.4)]"
