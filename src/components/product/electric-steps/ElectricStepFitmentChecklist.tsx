...
import { electricStepFitmentChecks } from "@/lib/electric-step-products";

export function ElectricStepFitmentChecklist() {
  return (
    <section
      className="py-16 sm:py-20 bg-zinc-900/50 border-t border-white/[0.05]"
      aria-labelledby="electric-step-checklist-title"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 lg:mb-10">
          ...
          <h2
            id="electric-step-checklist-title"
            className="text-2xl md:text-3xl font-bold text-white"
          >
            安装前必须确认的四个维度
          </h2>
          <p className="mt-3 text-zinc-400 max-w-2xl">
            电动踏板涉及底盘结构、电气系统和安全功能，每一项都直接影响使用安全和产品寿命。
          </p>
        </div>
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {electricStepFitmentChecks.map((item) => (
            ...
            >
              <span className="inline-flex w-fit items-center rounded-full bg-orange-400/10 px-2.5 py-0.5 text-[10px] font-semibold text-orange-300 mb-3 shadow-[0_0_0_1px_oklch(1_0_0/0.08)]">
                {item.label}
              </span>
            ...
            >
              <summary className="flex items-center justify-between p-5 cursor-pointer list-none">
                <div className="flex items-center gap-3">
                  <span className="inline-flex rounded-full bg-orange-400/10 px-2 py-0.5 text-[10px] font-semibold text-orange-300 shadow-[0_0_0_1px_oklch(1_0_0/0.08)]">
                    {item.label}
                  </span>
