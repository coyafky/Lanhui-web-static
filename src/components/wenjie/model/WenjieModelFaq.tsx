...
export type WenjieModelFaqProps = {
...
/**
 * 二级页 FAQ — M6 / M7 / M8 共用（Client Component）
 * 一次只展开一项；黑底圆角 2xl + 边框 zinc-800 + ChevronDown 旋转
 */
export function WenjieModelFaq({
  items,
  modelKey,
  modelName,
}: WenjieModelFaqProps) {
  ...
  return (
    <section
      ...
      aria-labelledby={`wenjie-${modelKey.toLowerCase()}-faq-heading`}
    >
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-expanded={open}
                  aria-controls={`wenjie-${modelKey.toLowerCase()}-faq-panel-${idx}`}
                >
                ...
                <div
                  id={`wenjie-${modelKey.toLowerCase()}-faq-panel-${idx}`}
