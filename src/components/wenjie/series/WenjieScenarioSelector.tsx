 ...
 */
export function WenjieScenarioSelector() {
  const [activeId, setActiveId] = useState<WenjieScenarioEntryId>(
    wenjieScenarioEntries[0].id,
  );
  const active =
    wenjieScenarioEntries.find((s) => s.id === activeId) ??
    wenjieScenarioEntries[0];
  const services = active.serviceIds
    .map((id) => wenjieBaseServices.find((s) => s.id === id))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));

  return (
    <section
      id="wenjie-scenarios"
      aria-labelledby="wenjie-scenarios-title"
            ...
            return (
              <button
                ...
                className={`flex min-h-11 flex-col items-start gap-1 rounded-2xl p-4 text-left transition-colors active:scale-[0.98] ${
                  isActive
                    ? "bg-cyan-400/10 shadow-[0_0_0_1px_oklch(0.789_0.154_211/0.4)]"
              ...
              >
                <span className="flex items-center gap-2">
                  {Icon && (
                    <Icon
                      className={`size-4 ${isActive ? "text-cyan-300" : "text-zinc-400"}`}
                      aria-hidden
                    />
                  )}
                  <span
                    className={`text-base font-semibold ${isActive ? "text-cyan-300" : "text-white"}`}
                  >
                    {s.title}
                  </span>
                </span>
