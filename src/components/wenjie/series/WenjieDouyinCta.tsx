...
import { wenjieDouyinHighlights } from "@/lib/wenjie-series-upgrade-projects";
...
export function WenjieDouyinCta() {
  return (
    <section
      aria-labelledby="wenjie-douyin-title"
      className="relative overflow-hidden bg-zinc-950 py-16 text-white sm:py-20 border-t border-white/[0.05]"
    >
      <div
        aria-hidden
        className="absolute -left-24 top-1/2 size-72 -translate-y-1/2 rounded-full bg-cyan-500/10 blur-3xl"
      />
      <div
        aria-hidden
        className="absolute -right-20 -top-24 size-80 rounded-full bg-cyan-400/10 blur-3xl"
      />
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:gap-16 lg:px-8">
        <div>
          <p className="mb-4 inline-flex min-h-10 items-center gap-2 rounded-full bg-cyan-400/10 px-4 text-sm font-medium text-cyan-300 shadow-[0_0_0_1px_oklch(1_0_0/0.08)]">
            <Video className="size-4" aria-hidden />
            官方抖音 · 蓝辉轻改
          </p>

          <h2
            id="wenjie-douyin-title"
          ...
          <p className="mt-5 max-w-xl text-pretty text-base leading-relaxed text-zinc-300 sm:text-lg">
            在蓝辉轻改官方抖音查看问界车膜、踏板和地板等项目的施工实拍视频。
          </p>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex min-h-12 select-none items-center justify-center gap-2 rounded-full bg-gradient-to-r from-cyan-500 to-cyan-600 pl-5 pr-[18px] text-base font-semibold text-white shadow-lg shadow-cyan-500/20 transition-[scale,box-shadow,filter] duration-150 ease-out hover:brightness-110 hover:shadow-cyan-400/30 active:scale-[0.96]"
            ...
            <div className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-white/[0.04] px-5 text-sm text-zinc-300 shadow-[0_0_0_1px_oklch(1_0_0/0.08)] sm:justify-start">
              <Search className="size-4 text-cyan-300" aria-hidden />
              抖音号
          ...
          <ul className="mt-8 grid gap-3 text-sm text-zinc-300 sm:grid-cols-3">
            {wenjieDouyinHighlights.map(({ iconName, label }) => {
              ...
              return (
                <li key={label} className="flex min-h-11 items-center gap-2">
                  <span className="inline-flex size-8 shrink-0 items-center justify-center rounded-lg bg-white/[0.05] shadow-[0_0_0_1px_oklch(1_0_0/0.08)]">
                    {Icon && <Icon className="size-4 text-cyan-300" aria-hidden />}
                  </span>
                  {label}
                </li>
              );
