...
import type { ZhijieV9FaqItem } from "@/lib/zhijie-v9-products";

type ZhijieV9FaqProps = {
  items: readonly ZhijieV9FaqItem[];
};

const FAQ_LENGTH = 6;

function assertFaqLength(items: readonly ZhijieV9FaqItem[]): void {
  if (items.length !== FAQ_LENGTH) {
    throw new Error(
      `ZhijieV9Faq expects ${FAQ_LENGTH} items, got ${items.length}`,
    );
  }
}

/**
 * 智界 V9 6 条 FAQ，结构和极氪 9X 保持一致。
 */
export function ZhijieV9Faq({ items }: ZhijieV9FaqProps) {
        ...
        <div className="space-y-3">
          {items.map((item, idx) => {
            const open = openIndex === idx;
                const panelId = `zhijie-v9-faq-panel-${idx}`;
                return (
              <div
                key={item.question}
