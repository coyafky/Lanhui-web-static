...
import { zhijieBaseServices } from "@/lib/zhijie-series-services";
...
/**
 * 6 类基础服务（智界全系均可咨询）：解决什么 / 适合谁 / 进入对应产品页。
 * 车膜类卡片包含车衣、隔热膜、改色膜 3 个子入口。
 */
export function ZhijieBaseServiceGrid() {
  return (
    <section
      aria-labelledby="zhijie-services-title"
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {zhijieBaseServices.map((svc) => {
            ...
            return (
              ...
              >
                <div className="size-10 rounded-xl bg-amber-400/10 flex items-center justify-center mb-4">
                ...
                {svc.subLinks ? (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {svc.subLinks.map((sub) => (
                      ...
                      >
                        {sub.label}
                        <ArrowRight className="size-3.5 text-amber-400" aria-hidden />
                      </Link>
                    ))}
                  </div>
                ) : (
                  <Link
                    href={svc.href}
                    className="mt-4 inline-flex min-h-11 items-center gap-2 self-start rounded-full bg-white/[0.04] px-5 text-sm font-medium text-amber-300 shadow-[0_0_0_1px_oklch(1_0_0/0.08)] transition-colors hover:bg-white/[0.08] active:scale-[0.96]"
                  >
                    了解{svc.title}
