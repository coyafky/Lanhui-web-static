...
import {
  zhijieScenarioEntries,
  zhijieBaseServices,
  type ZhijieScenarioEntryId,
 ...
 */
export function ZhijieScenarioSelector() {
  const [activeId, setActiveId] = useState<ZhijieScenarioEntryId>(
    zhijieScenarioEntries[0].id,
  );
  const active =
    zhijieScenarioEntries.find((s) => s.id === activeId) ??
    zhijieScenarioEntries[0];
  const services = active.serviceIds
    .map((id) => zhijieBaseServices.find((s) => s.id === id))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));

  return (
    <section
      id="zhijie-scenarios"
      aria-labelledby="zhijie-scenarios-title"
