...
import {
  gaoshanScenarioEntries,
  gaoshanBaseServices,
  type GaoshanScenarioEntryId,
 ...
 */
export function GaoshanScenarioSelector() {
  const [activeId, setActiveId] = useState<GaoshanScenarioEntryId>(
    gaoshanScenarioEntries[0].id,
  );
  const active =
    gaoshanScenarioEntries.find((s) => s.id === activeId) ??
    gaoshanScenarioEntries[0];
  const services = active.serviceIds
    .map((id) => gaoshanBaseServices.find((s) => s.id === id))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));

  return (
    <section
      id="gaoshan-scenarios"
      aria-labelledby="gaoshan-scenarios-title"
            ...
            return (
              <button
                ...
                className={`flex min-h-11 flex-col items-start gap-1 rounded-2xl p-4 text-left transition-colors active:scale-[0.98] ${
                  isActive
                    ? "bg-teal-400/10 shadow-[0_0_0_1px_oklch(0.704_0.14_182.5/0.4)]"
              ...
              >
                <span className="flex items-center gap-2">
                  {Icon && (
                    <Icon
                      className={`size-4 ${isActive ? "text-teal-300" : "text-zinc-400"}`}
                      aria-hidden
                    />
                  )}
                  <span
                    className={`text-base font-semibold ${isActive ? "text-teal-300" : "text-white"}`}
                  >
                    {s.title}
                  </span>
                </span>
        ...
        <div className="mt-6 rounded-3xl bg-zinc-900/60 p-6 shadow-[0_0_0_1px_oklch(1_0_0/0.06)]">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {services.map((svc) => (
              ...
              >
                {svc.title}
                <ArrowRight className="size-4 text-teal-400" aria-hidden />
              </Link>
            ))}
          </div>
