...
import type { GaoshanModelEntryKey } from "@/lib/gaoshan-series-services";

export type GaoshanModelEntry = {
  modelKey: GaoshanModelEntryKey;
  modelName: string;
...
type GaoshanModelEntryGridProps = {
  entries: readonly GaoshanModelEntry[];
};

/**
 * 高山 8 车型专属子页入口（整卡可点）。
 * 品牌页只保留入口，23 个专车项目在 /product/gaoshan/8 子页展示。
 * 底部提供"其他高山版本"咨询入口，不暗示全系通用。
 */
export function GaoshanModelEntryGrid({ entries }: GaoshanModelEntryGridProps) {
  return (
    <section
      id="gaoshan-models"
      aria-labelledby="gaoshan-models-title"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 lg:mb-10">
          ...
          <p className="mt-3 text-zinc-400 max-w-2xl text-base leading-relaxed text-pretty">
            高山 8 已整理独立专车方案。进入子页查看完整项目清单，按年款与配置继续确认。
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
            ...
            >
              ...
              <div className="flex flex-1 flex-col gap-3 p-5">
                <div className="flex items-center gap-3">
                  ...
                  <span className="rounded-full bg-teal-400/10 px-2.5 py-0.5 text-xs font-medium text-teal-300">
                ...
                <span className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-teal-400 transition-colors group-hover:text-teal-300">
                  查看 {m.modelName} 专属方案
          ...
          <div className="flex flex-col justify-center gap-4 rounded-3xl bg-zinc-900/60 p-6 shadow-[0_0_0_1px_oklch(1_0_0/0.06)]">
            ...
            <p className="text-base text-zinc-400 leading-relaxed text-pretty">
              高山不同版本在门体结构、座椅布局和尺寸上有差异。发来车型、年款和配置，
              我们先帮你确认哪些基础服务通用、哪些需要专车适配。
            </p>
            <button
              type="button"
              onClick={() => openWeChatModal()}
              className="inline-flex min-h-11 items-center gap-2 self-start rounded-full bg-gradient-to-r from-teal-500 to-teal-600 px-5 text-sm font-semibold text-white shadow-lg shadow-teal-500/20 transition-transform active:scale-[0.96]"
            >
