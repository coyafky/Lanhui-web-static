...
import type { NioModelEntryKey } from "@/lib/nio-series-services";

export type NioModelEntry = {
  modelKey: NioModelEntryKey;
  modelName: string;
...
type NioModelEntryGridProps = {
  entries: readonly NioModelEntry[];
};

/**
 * 蔚来 ES8 车型专属子页入口（整卡可点）。
 * 品牌页只保留入口，17 个专车项目在 /product/nio/es8 子页展示。
 * 底部提供"其他蔚来车型"咨询入口，不暗示全系通用。
 */
export function NioModelEntryGrid({ entries }: NioModelEntryGridProps) {
  return (
    <section
      id="nio-models"
      aria-labelledby="nio-models-title"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 lg:mb-10">
          ...
          <p className="mt-3 text-zinc-400 max-w-2xl text-base leading-relaxed text-pretty">
            ES8 已整理独立专车方案。进入子页查看完整项目清单，按年款与配置继续确认。
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
          {entries.map((m) => (
            ...
            >
              ...
              <div className="flex flex-1 flex-col gap-3 p-5">
                ...
                <span className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-sky-400 transition-colors group-hover:text-sky-300">
                  查看 {m.modelName} 专属方案
          ...
          <div className="flex flex-col justify-center gap-4 rounded-3xl bg-zinc-900/60 p-6 shadow-[0_0_0_1px_oklch(1_0_0/0.06)]">
            ...
            <p className="text-base text-zinc-400 leading-relaxed text-pretty">
              蔚来 SUV、轿车和旅行车在车身结构、感知区域和座舱布局上差异明显，脚垫、踏板和地板不能通用。
              发来车型、年款和配置，我们先帮你确认哪些基础服务通用、哪些需要专车适配。
            </p>
            <button
              type="button"
              onClick={() => openWeChatModal()}
              className="inline-flex min-h-11 items-center gap-2 self-start rounded-full bg-gradient-to-r from-sky-500 to-sky-600 px-5 text-sm font-semibold text-white shadow-lg shadow-sky-500/20 transition-transform active:scale-[0.96]"
            >
