...
import { nioBaseServices } from "@/lib/nio-series-services";
...
/**
 * 6 类基础服务（蔚来全系均可咨询）：解决什么 / 适合谁 / 进入对应产品页。
 * 车膜类卡片包含车衣、隔热膜、改色膜 3 个子入口。
 */
export function NioBaseServiceGrid() {
  return (
    <section
      aria-labelledby="nio-services-title"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 lg:mb-10">
          ...
          <p className="mt-3 text-zinc-400 max-w-2xl text-base leading-relaxed text-pretty">
            这些是蔚来车主最常做的方向。每类都链接到独立的专业页面，先了解清楚再决定。
          </p>
        </div>

        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {nioBaseServices.map((svc) => {
            ...
            return (
              ...
              >
                <div className="size-10 rounded-xl bg-sky-400/10 flex items-center justify-center mb-4">
                  {Icon && <Icon className="size-5 text-sky-400" aria-hidden />}
                </div>
                ...
                {svc.subLinks ? (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {svc.subLinks.map((sub) => (
                      ...
                      >
                        {sub.label}
                        <ArrowRight className="size-3.5 text-sky-400" aria-hidden />
                      </Link>
                    ))}
                  </div>
                ) : (
                  <Link
                    href={svc.href}
                    className="mt-4 inline-flex min-h-11 items-center gap-2 self-start rounded-full bg-white/[0.04] px-5 text-sm font-medium text-sky-300 shadow-[0_0_0_1px_oklch(1_0_0/0.08)] transition-colors hover:bg-white/[0.08] active:scale-[0.96]"
                  >
                    了解{svc.title}
