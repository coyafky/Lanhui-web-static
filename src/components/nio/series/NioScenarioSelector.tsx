 ...
 */
export function NioScenarioSelector() {
  const [activeId, setActiveId] = useState<NioScenarioEntryId>(
    nioScenarioEntries[0].id,
  );
  const active =
    nioScenarioEntries.find((s) => s.id === activeId) ?? nioScenarioEntries[0];
  const services = active.serviceIds
    .map((id) => nioBaseServices.find((s) => s.id === id))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));

  return (
    <section
      id="nio-scenarios"
      aria-labelledby="nio-scenarios-title"
            ...
            return (
              <button
                ...
                className={`flex min-h-11 flex-col items-start gap-1 rounded-2xl p-4 text-left transition-colors active:scale-[0.98] ${
                  isActive
                    ? "bg-sky-400/10 shadow-[0_0_0_1px_oklch(0.685_0.169_237.323/0.4)]"
              ...
              >
                <span className="flex items-center gap-2">
                  {Icon && (
                    <Icon
                      className={`size-4 ${isActive ? "text-sky-300" : "text-zinc-400"}`}
                      aria-hidden
                    />
                  )}
                  <span
                    className={`text-base font-semibold ${isActive ? "text-sky-300" : "text-white"}`}
                  >
                    {s.title}
                  </span>
                </span>
