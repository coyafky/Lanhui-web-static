...
- [x] 2.2 批量发布按钮 → 调用 `setPendingConfirm({ type: "bulk", action: "publish", ids })`
- [x] 2.3 批量归档按钮 → 调用 `setPendingConfirm({ type: "bulk", action: "archive", ids })`
- [x] 2.4 批量删除按钮 → 调用 `setPendingConfirm({ type: "bulk", action: "delete", ids })`
...
- [x] 5.1 `npx vitest run src/app/admin/(dashboard)/articles/page.test.tsx`
